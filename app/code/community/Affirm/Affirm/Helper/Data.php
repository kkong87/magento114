...
class Affirm_Affirm_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getExchange($orderid = 0){
        if(!$orderid){
            $result = $this->getOrder()->getPayment()->getMethodInstance()->getExchangeProvider();
            return $result;
        } else {
            $order = Mage::getModel('sales/order')->load($orderid);
            $result = $order->getPayment()->getMethodInstance()->getExchangeProvider();
            return $result;
        }
    }

    public function getExchangeID($orderid = 0){
            if(!$orderid){
                $result = $this->getOrder()->getPayment()->getMethodInstance()->getExchangeProviderID();
                return $result;
            } else {
                $order = Mage::getModel('sales/order')->load($orderid);
                $result = $order->getPayment()->getMethodInstance()->getExchangeProviderID();
                return $result;
            }
    }
}
