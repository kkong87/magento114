...
class Affirm_Affirm_Model_Order_Observer_AfterSaveOrder
{
    ...
    public function postDispatchSaveOrder($observer)
    {
        ...
        $exchangeID = $order->getPayment()->getMethodInstance()->getExchangeProviderID();

        if($exchangeID != '' && $exchangeID != 1) {
            $this->updateAuthAmount($order);
        }

        return;
    }

    private function updateAuthAmount($order){
        $total = $order->getData('base_grand_total');
        $chargeID = $order->getPayment()->getMethodInstance()->getChargeId();
        $comments = $order->getStatusHistoryCollection(true);
        foreach ($comments as $comment) {
            if (strpos($comment->getData('comment'), 'Authorized amount') !== false) {
                $comment->setData('comment' ,'Authorized amount of $'.$total.'. Transaction ID: "'.$chargeID.'".')->save();
            }
        }
    }
}
